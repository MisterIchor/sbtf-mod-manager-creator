...
func _update_visual(metadata: Dictionary) -> void:
	var image: Image = Image.load_from_file(metadata.path_to)
	selected_file_texture.texture = ImageTexture.create_from_image(image)
	
	if not metadata.replacer.is_empty():
		var replacer_image: Image = Image.load_from_file(metadata.replacer)
		replacer_texture.texture = ImageTexture.create_from_image(replacer_image)
	else:
		replacer_texture.texture = load("res://icon.svg")
...
func _on_visibility_changed() -> void:
	...
	var dirs_to_search: PackedStringArray = [SBTFTool.OUTPUT_PATH]
	...
	while not dirs_to_search.is_empty():
		...
		if current_file.get_extension() == "tga":
